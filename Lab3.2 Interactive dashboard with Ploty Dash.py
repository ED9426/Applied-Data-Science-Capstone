...
df = pd.DataFrame(spacex_df['Launch Site'].unique())
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                # The default select value is for ALL sites
                                  ...
                                  options=[{'label': 'All Sites', 'value': 'ALL'}] + pd.concat([df.rename(columns={0:'label'}), 
                                                                                                df.rename(columns={0:'value'})], axis=1).to_dict('records'),
                                  value='ALL',
...
def slider(entered_site, slider_value):
    df = spacex_df[spacex_df['Payload Mass (kg)'] >= slider_value[0]]
    df = df[df['Payload Mass (kg)'] <= slider_value[1]]
    if entered_site == 'ALL':
